...
export async function loginWithAwsOtp(sessionId: string, otp: string): Promise<LoginResponse> {
  const response = await fetch(`${API_BASE_URL}/api/auth/superadmin/login-aws-otp`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  if (!payload || !('token' in payload) || !payload.token) {
    throw new AuthError('OTP Login failed', response.status)
  }
