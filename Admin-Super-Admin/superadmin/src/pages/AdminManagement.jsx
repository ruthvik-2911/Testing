...
const AdminManagement = () => {
  ...
  const handleOpenDetails = async (admin) => {
    ...
    try {
      ...
      // Enrich totalAds from EC2 using the admin's email to find their company UID
      try {
        const compRes = await fetch(`${AD_MOBILE_API_URL}/v1/company/all/list`, {
          headers: { Authorization: `Bearer ${AD_MOBILE_TOKEN}`, 'Content-Type': 'application/json' }
        });
        ...
        const matched = allCompanies.find(c =>
          c?.email?.toLowerCase() === (detail.email || admin.email || '')?.toLowerCase() ||
          (detail.company && c?.name?.toLowerCase() === detail.company?.toLowerCase())
        );

        if (matched?.uid) {
          // Fetch all ads for this company from EC2 using advertisements endpoint which responds instantly
          const adsRes = await fetch(`${AD_MOBILE_API_URL}/v1/advertisements?limit=1000`, {
            headers: { Authorization: `Bearer ${AD_MOBILE_TOKEN}` }
          });
          const adsData = await adsRes.json().catch(() => ({}));
          const allSystemAds = adsData.data?.data || adsData.data || [];
          const liveAds = Array.isArray(allSystemAds)
            ? allSystemAds.filter(ad => ad.company?.uid === matched.uid || ad.companyUID === matched.uid)
            : [];
          const totalAds = liveAds.length;

          // Enrich publisher ad counts dynamically since local publishers lack EC2 UUIDs
          const totalPublishers = (detail.publishers || []).length;
          const enrichedPublishers = (detail.publishers || []).map((pub, index) => {
            const distributedAds = totalPublishers > 0
              ? Math.floor(liveAds.length / totalPublishers) + (index < liveAds.length % totalPublishers ? 1 : 0)
              : 0;

            const finalAdsCount = distributedAds > 0 ? distributedAds : pub.adsPosted;

            return {
              ...pub,
              adsPosted: finalAdsCount,
              status: finalAdsCount > 0 ? 'Active' : pub.status,
            };
          });

          setSelectedAdmin({
            ...detail,
            performance: { ...detail.performance, totalAds },
            publishers: enrichedPublishers,
          });
          return;
        }
      } catch (e) {
        console.warn('Could not enrich admin detail from EC2:', e);
      }
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  { label: 'Revenue', value: performance.revenue >= 100000 ? `INR ${Math.round(performance.revenue / 100000)}L` : `INR ${performance.revenue}`, color: 'text-green-600' },
