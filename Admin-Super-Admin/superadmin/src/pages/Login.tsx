...
import { AuthError, getAuthSession, loginSuperAdmin, persistAuthSession, sendSmsOtp, loginWithAwsOtp } from '../lib/auth'
...
export default function Login() {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Request OTP from backend (AWS SNS flow)
      const sid = await sendSmsOtp(formattedPhone)
      ...
      setLoading(false)
      console.error('AWS OTP Send Error:', error)
      setAuthMessage(error.message || 'Failed to send OTP')
    }
  }
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Call backend to verify OTP and get local JWT
      const response = await loginWithAwsOtp(sessionId, otpString)
      persistAuthSession(response)
      ...
      setLoading(false)
      console.error('AWS OTP Verify Error:', error)
      setAuthMessage(error.message || 'Invalid OTP code')
    }
  }
