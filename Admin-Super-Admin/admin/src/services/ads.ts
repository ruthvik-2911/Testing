...
export const finalizeAdPublication = async (
  ...
  // BINGO: After successful creation in EC2, we MUST scope it to the local admin account.
  // This ensures the dashboard and ads list are strictly isolated.
  const campaignId = response.data.data?.uid || response.data.data?._id;
  if (campaignId) {
    try {
      await api.post('/api/admin/ads/scope', { campaignId, advertisementId });
      console.log(`✅ Campaign ${campaignId} successfully scoped to admin.`);
    } catch (e) {
      console.error('❌ Failed to scope campaign to admin context:', e);
    }
  }
};
