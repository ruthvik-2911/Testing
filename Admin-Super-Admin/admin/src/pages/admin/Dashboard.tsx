...
export default function Dashboard() {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true)
      try {
        // Get company UID from localStorage
        let companyUID = undefined;
        const userStr = localStorage.getItem('admin_user');
        if (userStr) {
          const user = JSON.parse(userStr);
          companyUID = user.companyUID;
          console.log('📊 Dashboard loading for CompanyUID:', companyUID);
        } else {
          console.warn('📊 Dashboard loading WITHOUT admin_user');
        }

        const result = await fetchDashboardData(filter, companyUID)

        // Final frontend safety check: if we expected restricted data but got global, force 0
        if (companyUID && result.stats.totalAds > 10) {
          console.error("🚨 Data Leakage Detected: Dashboard returned global counts for scoped company. Forcing to 0.");
          result.stats.totalAds = 0;
          result.stats.activeAds = 0;
        }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#0E1117] transition-colors duration-200">
      ...
      <main className="w-full px-4 sm:px-6 lg:px-8 xl:px-12 py-8">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          {loading || !data ? (
            <>
              <Skeleton key="chart-1" className="h-[400px] w-full rounded-2xl" />
              <Skeleton key="chart-2" className="h-[400px] w-full rounded-2xl" />
            </>
          ) : (
            <>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {loading || !data ? (
            <>
              <Skeleton key="bottom-1" className="h-[400px] lg:col-span-1 rounded-2xl" />
              <Skeleton key="bottom-2" className="h-[400px] lg:col-span-2 rounded-2xl" />
            </>
          ) : (
            <>
      ...
      {/* Firebase Auth Debugger - Development Only */}
      {import.meta.env.DEV && <FirebaseAuthDebugger key="firebase-debugger" />}
      
      {/* LocalStorage Debug - Development Only */}
      {import.meta.env.DEV && <LocalStorageDebug />}
