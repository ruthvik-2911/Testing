...
public class AdminPublisherController {
    ...
    // ──────────────────────────────────────────────
    // GET All Publishers for Logged-In Admin (Production Data)
    // ──────────────────────────────────────────────
    @GetMapping
    public ResponseEntity<?> getPublishersForAdmin(
        ...
        try {
            ...
            System.out.println("====== FETCHING PUBLISHERS FROM PRODUCTION FOR ADMIN: " + adminId + " ======");
            
            // Fetch publishers from production Mobilize API
            List<Map<String, Object>> productionPublishers = mobilizeApiService.fetchPublishersFromProduction();
            System.out.println("====== FOUND " + productionPublishers.size() + " PRODUCTION PUBLISHERS ======");

            // Convert production data to Publisher model format
            List<Map<String, Object>> publishers = productionPublishers.stream()
                .map(company -> {
                    Map<String, Object> publisher = new HashMap<>();
                    publisher.put("id", company.get("_id"));
                    publisher.put("name", company.get("name"));
                    publisher.put("email", company.get("email"));
                    publisher.put("location", company.get("billingAddress") != null ? "Available" : "Not specified");
                    publisher.put("ads", company.get("status") != null ? ((Boolean) company.get("status")) ? "Active" : "Inactive" : "Unknown");
                    publisher.put("engagement", Math.random() * 100); // Mock engagement data
                    publisher.put("activity", "Recently Active");
                    publisher.put("adminId", adminId); // Set to current admin for permissions
                    return publisher;
                })
                .collect(Collectors.toList());

            return ResponseEntity.ok(Map.of("success", true, "publishers", publishers));
        } catch (Exception e) {
            System.err.println("Error fetching production publishers: " + e.getMessage());
    ...
    public ResponseEntity<?> getPublisherById(
        ...
        try {
            ...
            if (!opt.get().getAdminId().equals(adminId))
                return unauthorized();
        ...
        try {
            ...
            if (publisherRequest.getName() == null || publisherRequest.getName().trim().isEmpty())
                return ResponseEntity.badRequest()
                        .body(Map.of("success", false, "message", "Publisher Name is required"));
            ...
            return ResponseEntity
                    .ok(Map.of("success", true, "message", "Publisher created successfully", "publisher", publisher));
        } catch (Exception e) {
            return serverError(e);
        }
    }
    ...
    public ResponseEntity<?> updatePublisher(
        ...
        try {
            ...
            if (!publisher.getAdminId().equals(adminId))
                return unauthorized();
            ...
            publisherRepository.save(publisher);
            return ResponseEntity
                    .ok(Map.of("success", true, "message", "Publisher updated successfully", "publisher", publisher));
        } catch (Exception e) {
            return serverError(e);
        }
    }
    ...
    public ResponseEntity<?> getPublisherAnalytics(
        ...
        try {
            ...
            long impressions = nearbyHits.stream()
                    .filter(h -> "AD_VIEW".equalsIgnoreCase(h.getEventType())
                            || "PAGE_HIT".equalsIgnoreCase(h.getEventType()))
                    .count();
            ...
            List<PublisherAnalyticsResponse.CampaignData> campaignDataList = allCampaigns.stream()
                    ...
                    .map(c -> {
                        ...
                        cd.setId(c.getId());
                        cd.setTitle(c.getA() != null && c.getA().getTitle() != null ? c.getA().getTitle()
                                : "Campaign " + c.getId().substring(Math.max(0, c.getId().length() - 6)));
                        cd.setStatus(c.getCompaignsStatus() != null ? c.getCompaignsStatus() : "UNKNOWN");

                        if (c.getDateRange() != null) {
                            cd.setStartDate(
                                    c.getDateRange().getFromDate() != null
                                            ? c.getDateRange().getFromDate().toInstant().atZone(ZONE_ID).toLocalDate()
                                                    .format(dtf)
                                            : "-");
                            cd.setEndDate(c.getDateRange().getToDate() != null
                                    ? c.getDateRange().getToDate().toInstant().atZone(ZONE_ID).toLocalDate().format(dtf)
                                    : "-");
                        } else {
                            cd.setStartDate("-");
                            cd.setEndDate("-");
                        ...
                        long campClicks = nearbyHits.stream()
                                .filter(h -> c.getId().equals(h.getCampaignId())
                                        && "AD_CLICK".equalsIgnoreCase(h.getEventType()))
                                .count();
                        double campCtr = campImpressions > 0
                                ? Math.round((double) campClicks / campImpressions * 10000.0) / 100.0
                                : 0;
    ...
    private List<hitRecord> filterHitsNearPublisher(List<hitRecord> hits, Publisher publisher) {
        if (publisher.getLocation() == null || publisher.getLocation().trim().isEmpty())
            return hits;
    ...
    private String extractAdminId(String authHeader) {
        if (authHeader == null || !authHeader.startsWith("Bearer ")) {
            System.out.println("DEBUG: PUBLISHER API - Missing or invalid Auth Header: " + authHeader);
            return null;
        }
        try {
            String token = authHeader.substring(7);
            System.out.println("DEBUG: PUBLISHER API - Token received: [" + token + "]");
            Claims claims = jwtService.parseToken(token);
            return claims.get("userId", String.class);
        } catch (Exception e) {
            System.out.println("DEBUG: PUBLISHER API - Token Parse Error: " + e.getMessage());
            return null;
        }
    }
