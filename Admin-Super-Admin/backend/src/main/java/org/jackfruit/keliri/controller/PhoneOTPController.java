...
public class PhoneOTPController {
    ...
    @Autowired
    private FirebasePhoneOTPService firebasePhoneOTPService;
    ...
    public ResponseEntity<?> sendOTP(@RequestBody Map<String, String> request) {
        try {
            ...
            Map<String, Object> result = firebasePhoneOTPService.sendOTP(phoneNumber);
    ...
    public ResponseEntity<?> verifyOTP(@RequestBody Map<String, String> request) {
        try {
            ...
            Map<String, Object> result = firebasePhoneOTPService.verifyOTP(sessionId, otp, phoneNumber);
    ...
    public ResponseEntity<?> testFirebaseConnection() {
        try {
            boolean isInitialized = firebasePhoneOTPService.getFirebaseAuth() != null;
            
            if (isInitialized) {
                return ResponseEntity.ok(Map.of(
                    "success", true,
                    "message", "Firebase Phone OTP service is working",
                    "project", "mobilize-consumer-flutter-app"
                ));
            } else {
                return ResponseEntity.status(500)
                    .body(Map.of("success", false, "message", "Firebase not initialized"));
            }
        } catch (Exception e) {
            return ResponseEntity.status(500)
                .body(Map.of("success", false, "message", "Firebase test failed: " + e.getMessage()));
        }
    }
}
