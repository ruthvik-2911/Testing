...
public class SuperAdminAuthService {
    ...
    public Map<String, Object> login(String emailOrPhone, String password) {
        ...
        if (admin.getPassword() != null && passwordEncoder.matches(password, admin.getPassword())) {
            ...
            // Generate Local JWT token (24 hours validity)
            String localToken = jwtService.generateToken(admin);
    ...
    public Map<String, Object> loginWithAwsOtp(String sessionId, String otp) {
        // 1. Verify OTP
        if (!otpService.verifyOtp(sessionId, otp)) {
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Invalid or expired OTP");
        }

        // 2. Get phone number from session
        String phoneNumber = otpService.getPhoneNumberBySession(sessionId);
        if (phoneNumber == null) {
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Session expired or invalid");
        }

        // 3. Find admin by phone
        ...
        // 5. Success - Reset attempts and generate token
        admin.setRole(jwtService.normalizeRole(admin.getRole()));
        admin.setFailedAttempts(0);
        admin.setLocked(false);
        admin.setLockUntil(null);
        ...
        payload.put("message", "AWS OTP Login Successful");
        payload.put("expiresInMs", expirationMs);
        payload.put("expiresInHours", 24);
        payload.put("email", admin.getEmail());
        payload.put("name", admin.getName());
    ...
    public Map<String, Object> validateToken(String token) {
        try {
            ...
            boolean isValid = true; // If parsing succeeded, it's valid for now in this context
            boolean expiringSoon = false; // Simplified
            ...
            result.put("remainingHours", 24); // Placeholder
