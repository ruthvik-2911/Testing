...
public class SuperAdminAnalyticsService {
    ...
    public SuperAdminAnalyticsResponse getAnalytics() {
        // Fetch data from Mobilize Production API
        List<Map<String, Object>> campaignsData = mobilizeApiService.fetchCampaignsFromProduction();
        List<Map<String, Object>> advertisementsData = mobilizeApiService.fetchAdvertisementsFromProduction();
        List<Map<String, Object>> publishersData = mobilizeApiService.fetchPublishersFromProduction();
        
        System.out.println("📊 Fetched " + campaignsData.size() + " campaigns from Production API");
        System.out.println("📺 Fetched " + advertisementsData.size() + " advertisements from Production API");
        System.out.println("👥 Fetched " + publishersData.size() + " publishers from Production API");
        
        // Convert to domain objects for compatibility
        List<ad_campaigns> campaigns = convertToCampaigns(campaignsData);
        List<users> publishers = convertToUsers(publishersData);
        Map<String, advertisements> adsById = loadAdvertisementsFromData(advertisementsData);
        Map<String, users> creatorsById = loadCreatorsFromData(publishersData);
        Map<String, txn_user_locations> publisherLocations = loadPublisherLocationsFromData(publishers);
    ...
    // Conversion methods for Mobilize data
    private List<ad_campaigns> convertToCampaigns(List<Map<String, Object>> campaignsData) {
        return campaignsData.stream().map(this::convertToCampaign).collect(Collectors.toList());
    }

    private ad_campaigns convertToCampaign(Map<String, Object> data) {
        ad_campaigns campaign = new ad_campaigns();
        campaign.setId(getString(data, "_id"));
        campaign.setAdvertisementId(getString(data, "advertisementId"));
        campaign.setCreatedBy(getString(data, "createdBy"));
        campaign.setCompaignsStatus(getString(data, "compaignsStatus"));
        
        // Convert location
        if (data.containsKey("location")) {
            Map<String, Object> locationData = (Map<String, Object>) data.get("location");
            location loc = new location();
            loc.setLocationName(getString(locationData, "locationName"));
            loc.setLat(getString(locationData, "lat"));
            loc.setLng(getString(locationData, "lng"));
            loc.setRange(getDouble(locationData, "range"));
            campaign.setLocation(loc);
        }
        
        // Convert date range
        if (data.containsKey("dateRange")) {
            Map<String, Object> dateData = (Map<String, Object>) data.get("dateRange");
            dateRange range = new dateRange();
            if (dateData.containsKey("startDate")) {
                range.setFromDate(parseDate(getString(dateData, "startDate")));
            }
            if (dateData.containsKey("endDate")) {
                range.setToDate(parseDate(getString(dateData, "endDate")));
            }
            campaign.setDateRange(range);
        }
        
        return campaign;
    }

    private List<users> convertToUsers(List<Map<String, Object>> usersData) {
        return usersData.stream().map(this::convertToUser).collect(Collectors.toList());
    }

    private users convertToUser(Map<String, Object> data) {
        users user = new users();
        user.setId(getString(data, "_id"));
        user.setFullName(getString(data, "companyName"));
        user.setEmailAddress(getString(data, "emailId"));
        return user;
    }

    private Map<String, advertisements> loadAdvertisementsFromData(List<Map<String, Object>> advertisementsData) {
        return advertisementsData.stream()
                .collect(Collectors.toMap(
                    ad -> getString(ad, "_id"),
                    ad -> convertToAdvertisement(ad)
                ));
    }

    private advertisements convertToAdvertisement(Map<String, Object> data) {
        advertisements ad = new advertisements();
        ad.setId(getString(data, "_id"));
        ad.setTitle(getString(data, "title"));
        ad.setAdType(getString(data, "adType"));
        return ad;
    }

    private Map<String, users> loadCreatorsFromData(List<Map<String, Object>> usersData) {
        return usersData.stream()
                .collect(Collectors.toMap(
                    user -> getString(user, "_id"),
                    user -> convertToUser(user)
                ));
    }

    private Map<String, txn_user_locations> loadPublisherLocationsFromData(List<users> publishers) {
        // For now, return empty map as location data might need separate handling
        return new HashMap<>();
    }
    ...
    private Date parseDate(String dateString) {
        try {
            return java.text.DateFormat.getInstance().parse(dateString);
        } catch (Exception e) {
            return new Date();
        }
    }
